...
pub fn LoginPage(cx: Scope) -> Element {
    let form_state = use_state(cx, LoginState::default);
    let navigator = use_navigator(cx);
    let notifications = use_state::<Vec<Notification>>(cx, Vec::new);
    ...
    let handle_submit = move |event: FormEvent| {
        ...
        async {
            ...
            let result = simulate_login_api(&form_state.get().data).await;
    ...
    cx.render(rsx! {
        div { class: "w-full bg-gray-50 dark:bg-gray-900 min-h-screen",
            div { class: "pt:mt-0 mx-auto flex flex-col items-center justify-center px-6 pt-8 md:h-screen dark:bg-gray-900",
                Link { to: "/", class: "my-2 flex items-center justify-center pt-5 text-2xl font-semibold lg:mb-10 dark:text-white",
                ...
                div { class: "flex w-full max-w-xl flex-col divide-gray-200 rounded-lg border-gray-200 bg-white p-4 text-gray-500 shadow-md sm:p-6 dark:divide-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400",
                    h1 { class: "text-2xl font-bold text-gray-900 dark:text-white", "Sign in to platform" }
                    ...
                    form {
                        ...
                        div { class: "flex items-start",
                            label { class: "flex items-center text-sm font-medium text-gray-900 accent-primary-600 rtl:text-right dark:text-gray-300",
                                input {
                                    r#type: "checkbox",
                                    value: "on",
            ...
            if form_state.get().not_verified {
                div { class: "alert alert-error w-full max-w-lg shadow-lg mx-auto mt-4",
                    div { class: "flex items-center",
                        ...
                        span { class: "ml-2", "{messages["notVerified"]}" }
                    }
                }
            }
                ...
                notifications.get().iter().map(|notification| {
                    ...
                    rsx! {
                        div {
                            ...
                            class: "rounded-md p-4 text-white {bg_color} shadow-lg",
                            onmounted: move |_| {
