...
pub fn Register() -> Element {
    let mut form_state = use_signal(RegisterState::default);
    ...
    let handle_submit = move |event: FormEvent| {
        ...
        if form_state.read().data.password != form_state.read().data.confirm_password {
        ...
        if !form_state.read().data.accept_terms {
            form_state.with_mut(|state| {
        ...
        dioxus::prelude::spawn({
            ...
            async move {
                ...
                let result = simulate_register_api(&form_state.read().data).await;
    ...
    let mut handle_input_change = move |field: &str, value: String| {
        form_state.with_mut(|state| {
    ...
    let mut handle_terms_change = move |checked: bool| {
        form_state.with_mut(|state| {
            ...
            div { class: "pt:mt-0 mx-auto flex flex-col items-center justify-center px-6 pt-8 md:h-screen dark:bg-gray-900",
                ...
                div { class: "flex w-full max-w-xl flex-col divide-gray-200 rounded-lg border-gray-200 bg-white p-4 text-gray-500 shadow-md sm:p-6 dark:divide-gray-700 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400",
                    ...
                    form {
                        ...
                        // Name field
                        div {
                            label { class: "block space-y-2 text-sm font-medium text-gray-900 rtl:text-right dark:text-white",
                                span { "Name" }
                                div { class: "relative w-full",
                                    input {
                                        ...
                                        required: true,
                                        value: "{form_state.read().data.name}",
                                    ...
                                    if let Some(err) = &form_state.read().errors.name {
                        ...
                        div {
                            label { class: "block space-y-2 text-sm font-medium text-gray-900 rtl:text-right dark:text-white",
                                ...
                                div { class: "relative w-full",
                                    ...
                                    if let Some(err) = &form_state.read().errors.email {
                                        p { class: "mt-2 text-sm text-red-600 dark:text-red-500",
                                            "{err}"
                                        }
                                    }
                            ...
                            label { class: "block space-y-2 text-sm font-medium text-gray-900 rtl:text-right dark:text-white",
                                ...
                                div { class: "relative w-full",
                                    input {
                                        ...
                                        required: true,
                                        r#type: "password",
                                        value: "{form_state.read().data.password}",
                                        oninput: move |e| handle_input_change("password", e.value().clone()),
                                        class: "w-full rounded border border-gray-300 bg-gray-50 p-2.5 text-gray-900 focus:border-primary-600 focus:ring-primary-600 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500 sm:text-sm",
                                    }
                                    div { class: "absolute inset-y-0 end-0 flex items-center pe-2.5 text-gray-500 dark:text-gray-400" }
                                    if let Some(err) = &form_state.read().errors.password {
                                        p { class: "mt-2 text-sm text-red-600 dark:text-red-500",
                                            "{err}"
                                        }
                                    }
                            ...
                            label { class: "block space-y-2 text-sm font-medium text-gray-900 rtl:text-right dark:text-white",
                                ...
                                div { class: "relative w-full",
                                    input {
                                        ...
                                        required: true,
                                        r#type: "password",
                                        value: "{form_state.read().data.confirm_password}",
                                        oninput: move |e| handle_input_change("confirm_password", e.value().clone()),
                                        class: "w-full rounded border border-gray-300 bg-gray-50 p-2.5 text-gray-900 focus:border-primary-600 focus:ring-primary-600 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-primary-500 dark:focus:ring-primary-500 sm:text-sm",
                                    }
                                    div { class: "absolute inset-y-0 end-0 flex items-center pe-2.5 text-gray-500 dark:text-gray-400" }
                                    if let Some(err) = &form_state.read().errors.confirm_password {
                                        p { class: "mt-2 text-sm text-red-600 dark:text-red-500",
                                            "{err}"
                                        }
                                    }
                        ...
                        // Terms checkbox
                        label { class: "flex items-center pt-1 text-sm font-medium text-gray-900 rtl:text-right dark:text-gray-300",
                            input {
                                r#type: "checkbox",
                                name: "accept",
                                checked: form_state.read().data.accept_terms,
                            ...
                            if let Some(err) = &form_state.read().errors.accept_terms {
                                p { class: "mt-2 text-sm text-red-600 dark:text-red-500",
                                    "{err}"
                                }
                            }
                        ...
                        button {
                            r#type: "submit",
                            disabled: form_state.read().loading,
